...
const FeedbackModal = () => {
  ...
  useEffect(() => {
    ...
    const isSubmitted = localStorage.getItem('studyhub_feedback_submitted') === 'true'
    const isDismissed = sessionStorage.getItem('studyhub_feedback_dismissed') === 'true'
    
    if (isSubmitted || isDismissed) return

    // Retrieve login count
    const countStr = localStorage.getItem('loginCount') || '0'
    const loginCount = parseInt(countStr)
  ...
  const handleSubmit = (e) => {
    ...
    setTimeout(() => {
      ...
      localStorage.setItem('studyhub_feedback_data', JSON.stringify(feedbackData))
      localStorage.setItem('studyhub_feedback_submitted', 'true')
  ...
  const handleDismiss = () => {
    ...
    sessionStorage.setItem('studyhub_feedback_dismissed', 'true')
    setIsOpen(false)
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
      ...
      {/* Modal box */}
      <div className="relative bg-white rounded-2xl shadow-2xl max-w-md w-full p-6 z-10 border border-gray-150 transform transition-all duration-300 scale-100 animate-in fade-in zoom-in-95">
        ...
        <form onSubmit={handleSubmit} className="space-y-4 text-xs">
          ...
          <div className="flex gap-3 pt-2">
            <button
              ...
              className="flex-1 py-2 bg-gray-150 hover:bg-gray-200 text-gray-700 rounded-lg font-semibold transition-colors text-center"
            >
              Ask Me Later
            </button>
            <button
